...
-- 导出  表 adolph.t_consistency_transaction 结构
CREATE TABLE IF NOT EXISTS `t_consistency_transaction` (
  `txId` bigint(20) NOT NULL COMMENT '事务ID',
  `checkUri` varchar(100) NOT NULL COMMENT '校验事务发起者是否成功的URL',
  `status` tinyint(4) NOT NULL COMMENT '0 新注册的事务 1 已经添加了事务参与者 2 消息待发送 3 已经发送过，但是不保证发送成功 4 发送成功 ',
  `createTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updateTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`txId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='最终一致性事务表';
